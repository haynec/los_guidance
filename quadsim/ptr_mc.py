...
from quadsim.discretization import calculate_discretization, s_to_t
...
from quadsim.plotting import save_gate_parameters

from copy import deepcopy

def PTR_main(params, monte_carlo = False):
    J_vb = 1E2
    J_vc = 1E2
    J_tr = 1E2
    ...
    if params.warm.warm_start:
        x_bar, u_bar = scp_init_warm(params.warm.warm_x_3dof, params.warm.warm_u_3dof, params.scp.n, gates, subs, params)
    ...
    if monte_carlo:
        w_tr_vec = params.scp.w_tr * np.logspace(-0.1, 0.1, 5)
        lam_t_vec = params.scp.lam_t * np.logspace(-0.3, 0.0, 5)
        lam_fuel_vec = params.scp.lam_fuel * np.logspace(-0.4, 0.0, 5)
        los_vio_vec = []
        los_vio_node_vec = []
        runtime_vec = []
        obj_vec = []
        ...
        w_tr_vec = [params.scp.w_tr]
        lam_t_vec = [params.scp.lam_t]
        lam_fuel_vec = [params.scp.lam_fuel]

    x_bar_org = x_bar
    u_bar_org = u_bar
    
    param_iter = 0
    for lam_t in lam_t_vec:

        params_iter = deepcopy(params)
        params_iter.scp.lam_fuel = lam_fuel_vec[param_iter]
        params_iter.scp.lam_t = lam_t
        param_iter += 1
        t_0_while = time.time()
        x_bar = x_bar_org
        u_bar = u_bar_org
        while k <= params_iter.scp.k_max and ((J_tr >= params_iter.scp.ep_tr) or (J_vb >= params_iter.scp.ep_vb) or (J_vc >= params_iter.scp.ep_vc) or (J_vc_ctcs >= params_iter.scp.ep_vc_ctcs)):
            x, u, t, J_total, J_vb_vec, J_vc_vec, J_tr_vec, J_vc_ctcs_vec, prob_stat = PTR_subproblem(cpg_solve, x_bar, u_bar, A, B, obstacles, subs, prob, sub_jax, quad_jax, params_iter)
            ...
            params_iter.scp.w_tr = min(params_iter.scp.w_tr * params_iter.scp.w_tr_adapt, params_iter.scp.w_tr_max)
            if k > params_iter.scp.free_final_drop:
                params_iter.scp.lam_t = params_iter.scp.lam_t * params_iter.scp.min_time_relax
            ...
            J_tr_colored = colored("{:.1e}".format(J_tr), col_pos if J_tr <= params_iter.scp.ep_tr else col_neg)
            J_vb_colored = colored("{:.1e}".format(J_vb), col_pos if J_vb <= params_iter.scp.ep_vb else col_neg)
            J_vc_colored = colored("{:.1e}".format(J_vc), col_pos if J_vc <= params_iter.scp.ep_vc else col_neg)
            J_vc_ctcs_colored = colored("{:.1e}".format(J_vc_ctcs), col_pos if J_vc_ctcs <= params_iter.scp.ep_vc_ctcs else col_neg)
        ...
        x_full = simulate_nonlinear(x[:,0], u, subs, sub_jax, params_iter)
        x_full_node = simulate_nonlinear(x[:,0], u, subs, sub_jax, params_iter, True).T
        u_full = interpolate_control(u, params_iter)

        x_sub_full = []
        if len(subs) != 0:
            x_sub_full, _, x_sub_sen = full_subject_traj(u, x_full, u_full, subs, params_iter, False)
            x_sub_sen_node = subject_traj(x, u, subs, params_iter)
        else:
            x_sub_sen = []
            x_sub_sen_node = []
        t_full = full_time(u, params_iter)

        obs_vio, sub_vp_vio, sub_vp_vio_node, sub_min_vio, sub_max_vio, sub_direc_vio, state_bound_vio = nonlinear_constraint(x_full, x, t_full, t, obstacles, subs, sub_jax, params_iter)
        ...
        if monte_carlo:
            los_vio_vec.append(LoS_vio)
            los_vio_node_vec.append(LoS_vio_node)
            runtime_vec.append(t_f_while - t_0_while)
            obj_vec.append(obj)
            iters.append(k)
    ...
    if monte_carlo:
        mc_results = dict(
            los_vio_vec = los_vio_vec,
            los_vio_node_vec = los_vio_node_vec,
    ...
    return result, LoS_vio, LoS_vio_node, t_f_while - t_0_while, k, obj, mc_results
...
def PTR_subproblem(cpg_solve, x_bar, u_bar, A, B, obstacles, subs, prob, sub_jax, quad_jax, params):
    ...
    if not params.scp.ctcs:
        ...
        for sub in subs:
            ...
            if params.vp.tracking:
                ...
                prob.param_dict['grad_g_bar_min_' + str(i_sub)].value = sub.grad_g_bar_sub_min_ctcs(np.array(t), x_bar.T, params, 'Full').T
                prob.param_dict['g_bar_max_' + str(i_sub)].value = sub.g_bar_sub_max_ctcs(np.array(t), x_bar.T, params, 'Full')
                prob.param_dict['grad_g_bar_max_' + str(i_sub)].value = sub.grad_g_bar_sub_max_ctcs(np.array(t), x_bar.T, params, 'Full').T
            i_sub += 1
    ...
    for k in range(params.scp.n):
        ...
        if not params.scp.ctcs:
            for obs in obstacles:
                J_vb += max(0, (obs.g_bar_obs(np.expand_dims(x_bar[:3,k], 1)) + obs.grad_g_bar_obs(np.expand_dims(x_bar[:3,k], 1)).T @ (x[:3,k] - x_bar[:3,k]))[0])
            for sub in subs:
